...
state_3 <- polls_plus(
  presidential_polls %>% filter(
    population %in% c("lv", "rv")),
            electoral_votes_2016, k=5) %>%
...
state_3$state_district <- as.character(state_3$state)

bead_snake_plot(
  state_3 %>% select(state_district, electoral_votes,
                     perc_dem, perc_rep), 
  height = 30, buffer = 3) +
